...
export class NavbarComponent implements OnInit {
  ...
  constructor(
    ...
    this.spinner.show();
    if (
      this.route.snapshot["_routerState"].url === "/login" ||
      this.route.snapshot["_routerState"].url === "/registration"
    ) {
      this.notLogin = true;
    } else {
      this.notLogin = false;
  ...
  unSubscribe() {
    if (this.isEdit) {
      this.confirmationService.confirm({
        message:
          "Are you sure  you want to unsubscribe this creator , As your subscription is still ACTIVE?",
        accept: () => {
          this.commonservice.getProfileID.subscribe((res1) => {
            this.commonservice.unSubscribe(res1).subscribe(
              (res: any) => {
                this.toastr.success("Successfully Unsubscribed.");
                this.commonservice
                  .user_log({ message: "User unsubscribe successfully!" })
                  .subscribe((res) => {});
              },
              (err) => {
  ...
  onTip(key, temp) {
    if (this.tipAmount > 0) {
      ...
      this.payQuantity = 1;
      this.commonservice.getCardDetail().subscribe(
        (res: any) => {
          if (res) {
            this.showSpinner = false;
            this.paymentProfileID =
              res.profile.paymentProfiles[0].customerPaymentProfileId;
            this.customerProfileID = res.profile.customerProfileId;
            this.cardNumber = res.profile.paymentProfiles[0].payment.creditCard.cardNumber
              .split("X")
              .pop();
  ...
  async onDownloadMedia(files) {
    ...
    return new Promise((pass, fail) => {
      this.commonservice
        .getDownloadPresignURL("download", files)
        .subscribe((data: any) => {
          if (data.url) {
            pass(data.url);
          }
  ...
  card() {
    ...
    this.commonservice.cardDeatil(obj).subscribe(
      (res: any) => {
        this.showSpinner = false;
        ...
        this.commonservice.getProfileID.subscribe((res1) => {
          if (this.payType === "subscription_upgrade" && this.isUpgrade) {
            this.commonservice
              .upgradeSubscription(res1, this.payQuantity, res.transId)
              .subscribe(
                (res) => {
                  this.toastr.success("Subscription updated successfully");
                  this.commonservice
                    .user_log({
                      message: "User update subscription successfully!",
                    })
                    .subscribe((res) => {});
                  this.modalRef.hide();
                  this.modalRef = null;
          ...
          if (this.payType === "subscription" && !this.isUpgrade) {
            this.commonservice
              .subscribeWithPurchase(res1, this.tierID.tier_id, res.transId)
              .subscribe(
                (res) => {
                  this.toastr.success("Subscribe Successfully");
                  this.commonservice
                    .user_log({ message: "User subscribe successfully!" })
                    .subscribe((res) => {});
                  this.modalRef.hide();
                  this.modalRef = null;
                },
    ...
    setInterval(() => {
      if (this.isCreator) {
        this.commonservice
          ...
          .then((res: any) => {
            if (
              res["ReceiveMessageResponse"]["ReceiveMessageResult"][
                "messages"
              ] !== null
            ) {
    ...
    if (valid) {
      ...
      let obj = {
        ...
        card_info: {
          ...
          expirationDate: moment(this.newCard.value.newExpirydate).format(
            "YYYY-MM"
          ),
      ...
      this.commonservice.addNewDetails(obj).subscribe(
        (res: any) => {
          this.showSpinner = false;
          this.toastr.success("Card Added successfully");
          this.commonservice
            .user_log({ message: "User added card successfully!" })
            .subscribe((res) => {});
          this.commonservice.getProfileID.subscribe((res1) => {
            if (this.payType === "subscription_upgrade" && this.isUpgrade) {
              this.commonservice
  ...
  freeSubscription() {
    this.commonservice.getProfileID.subscribe((res1) => {
      this.commonservice.freeSubscription(res1).subscribe(
        (res) => {
          this.toastr.success("Subscribe for free.");
          this.commonservice
            .user_log({ message: "User free subcribe successfully!" })
            .subscribe((res) => {});
          this.modalRef.hide();
          this.modalRef = null;
        },
  ...
  logout() {
    this.commonservice
      .logout({ access_token: localStorage.getItem("access_token") })
      .subscribe(
        (res) => {
          this.commonservice
            .user_log({ message: "User logout successfully!" })
            .subscribe((res) => {});
    ...
    if (valid) {
      ...
      let obj = {
        ...
        creditCard: {
          ...
          expirationDate: moment(this.cardForm.value.expirydate).format(
            "YYYY-MM"
          ),
      ...
      this.commonservice.cardDeatil(obj).subscribe(
        (res: any) => {
          ...
          this.toastr.success("Card Detail Added successfully");
          this.commonservice
            .user_log({ message: "User added card details successfully!" })
            .subscribe((res) => {});
          this.commonservice.getProfileID.subscribe((res1) => {
            this.commonservice
              .subscribeWithPurchase(res1, this.tierID.tier_id, res.transId)
  ...
  onUpdate(data, temp) {
    ...
    if (this.remainingDays > 0) {
      this.netCost =
        (this.perdayCostNew - this.perdayCostOld) * this.remainingDays +
        this.perdayCostNew * this.usedDays;
      if (this.netCost - this.currentSubscription.tier_price > 3) {
        ...
        Swal.fire({
          title: "Error",
          text:
            "you are not eligible for price discount as you have used used subscription  above discount limit.",
          icon: "error",
        });
      }
    } else {
